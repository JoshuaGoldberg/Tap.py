...
class InfoUI:
    ...
    def draw(self, surface, layer):
        ...
        if self.game.selectedWorker is not None:
            worker_image = Button(self.offset[0] + 148, self.offset[1] - 430, self.game.selectedWorker.image, 2.0, lambda: None, None, layer)
            worker_image.draw(surface, layer)
            name_text = font.render("Name: " + self.game.selectedWorker.firstname + " " + self.game.selectedWorker.lastname, True, (0, 0, 0))
